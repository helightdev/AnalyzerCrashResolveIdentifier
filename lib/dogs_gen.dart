...
String getLibraryName(Uri uri) {
  ...
  libraryName =
      libraryName.substring(0, libraryName.length - 5); // Remove .dart
  ...
  FutureOr<void> buildDeclarationsForClass(ClassDeclaration clazz,
    ...
    builder.declareInType(DeclarationCode.fromParts([
      ...
      ...fields.mapIndexed((i, e) {
        ...
        if (e.type.isNullable) {
          return RawCode.fromParts([
            "if (${e.identifier.name} != #not_changed) values[$index] = ${e
                .identifier.name};"
          ]);
        } else {
          return RawCode.fromParts([
            "if (${e.identifier.name} != null) values[$index] = ${e.identifier
                .name};"
          ]);
        }
      }),
...
macro class DefaultsMacro implements ClassDeclarationsMacro {
  ...
  const DefaultsMacro(this.uri, this.name);
  ...
  FutureOr<void> buildDeclarationsForClass(ClassDeclaration clazz,
    ...
    // Delegating super method
    builder.declareInType(DeclarationCode.fromParts([
      "${clazz.identifier.name}.root({",
      ...fields.map((e) =>
          RawCode.fromParts([
            (e.type.code.isNullable ? "" : "required "),
            e.type.code,
            " ${e.identifier.name}"
          ])).commaDelimited,
      "}) : super.\$args(",
      ...fields
          .map((e) => "${e.identifier.name}: ${e.identifier.name}")
          .commaDelimited,
      ");"
    ]));

    List<Code> fieldDefaults = [];
    for (var field in fields) {
      var hasDefaultAnnotation = field.metadata.any((element) {
        if (element is ConstructorMetadataAnnotation) {
          //TODO: Don't only check the name
          return element.type.identifier.name == 'Default';
        }
        return false;
      });
      if (hasDefaultAnnotation) {
        fieldDefaults.add((RawCode.fromParts([
          NamedTypeAnnotationCode(name: dartObject).asNullable,
          " ${field.identifier.name} = #not_changed"
        ])));
    ...
    List<Code> fieldInitializers = [];
    for (var field in fields) {
      Code? defaultsCode;
      var defaultsAnnotation = field.metadata.firstWhereOrNull((e) =>
      e is ConstructorMetadataAnnotation && e.type.identifier.name == 'Default');
      if (defaultsAnnotation != null) {
        defaultsCode = recreateAnnotation(defaultsAnnotation);
      }
      if (defaultsCode != null) {
        fieldInitializers.add(RawCode.fromParts([
          field.identifier.name,
          ": ",
          field.identifier.name,
          " == #not_changed ? ",
    ...
    builder.declareInType(DeclarationCode.fromParts([
      ...
      ...fieldDefaults.commaDelimited,
      "}) => ${clazz.identifier.name}.root(",
      ...fieldInitializers.commaDelimited,
      ");"
    ]));
  }
}
...
macro class Model implements ClassDeclarationsMacro, ClassDefinitionMacro, ClassTypesMacro {
  ...
  FutureOr<void> buildTypesForClass(ClassDeclaration clazz,
    ...
    var modelBuilderMacro = await builder.resolveIdentifier(
        _thisLibrary, 'ModelBuilderMacro');
    var copyWithFrontendMacro = await builder.resolveIdentifier(
        _thisLibrary, 'CopyWithFrontendMacro');
    var copyWithImplMacro = await builder.resolveIdentifier(
        _thisLibrary, 'CopyWithImplMacro');
    var defaultsMacro = await builder.resolveIdentifier(
        _thisLibrary, 'DefaultsMacro');

    var defaultsFactory = "${clazz.identifier.name}Defaults";
    builder.declareType(defaultsFactory, DeclarationCode.fromParts([
      "@",
      NamedTypeAnnotationCode(name: defaultsMacro),
      "('",
      clazz.library.uri.toString(),
      "','",
      clazz.identifier.name,
      "') ",
      "class $defaultsFactory extends ${clazz.identifier.name} {",
  ...
  FutureOr<void> buildDeclarationsForClass(ClassDeclaration clazz,
    ...
    deepEquality, overrideAnnotation, dartObject, dartInt] = await Future.wait([
      ...
      builder.resolveIdentifier(
          _structureConverterFile, 'DogStructureConverterImpl'),
      builder.resolveIdentifier(
          clazz.library.uri, "${clazz.identifier.name}Builder"),
    ...
    for (var field in fields) {
      ...
      if (hasDefaultAnnotation || field.type.isNullable) {
        fieldDefaults.add((RawCode.fromParts([
          field.type.code.asNullable,
          " ${field.identifier.name}"
        ])));
    ...
    // Declare constructor factory
    builder.declareInType(DeclarationCode.fromParts([
      "factory ${clazz.identifier.name}({",
      ...fieldDefaults.commaDelimited,
      "}) = ${clazz.identifier.name}Defaults;",
    ]));
    ...
    // Copy with
    var copyWithFrontend = await builder.resolveIdentifier(
        clazz.library.uri, '_${clazz.identifier.name}CopyWithFrontend');
    var copyWithImpl = await builder.resolveIdentifier(
        clazz.library.uri, '_${clazz.identifier.name}CopyWithImpl');
    builder.declareInType(DeclarationCode.fromParts([
    ...
    if (!hasDefaultConstructor) {
      builder.declareInType(DeclarationCode.fromParts([
        ...
        ".\$args({",
        ...fields
            .map((e) => "${e.type.isNullable ? "" : "required "}this.${e
            .identifier.name}")
            .commaDelimited,
        "});"
      ]));
    ...
    builder.declareInType(DeclarationCode.fromParts([
      ...
      " list) : ",
      ...fields
          .mapIndexed((i, e) => "${e.identifier.name} = list[$i]")
          .commaDelimited,
      ";"
    ]));
  ...
  FutureOr<void> buildDefinitionForClass(ClassDeclaration clazz,
    ...
    try {
      ...
      var retainedAnnotationStatic = await builder.resolve(
          NamedTypeAnnotationCode(name: retainedAnnotation));
      var propertyBuilder = await builder.buildMethod(
          structureGetter.identifier);
      ...
      for (var field in structureFields) {
        fieldDefinitions.add(RawCode.fromParts([
          ...
          "(",
          createLyellTypeTree(
              field.type, qualifiedTerminal, qualifiedTypeTreeN),
          ",",
          createLyellTypeTree(
              field.type, qualifiedTerminal, qualifiedTypeTreeN),
          ",null,",
          ...
          ",",
          (!(await isSimpleType(field.type.code, builder, primitiveRecord)))
              .toString(),
          ",",
          await createMetaList(
              field.metadata, retainedAnnotationStatic, builder),
          ")"
        ]));
      }
      ...
      propertyBuilder.augment(FunctionBodyCode.fromParts([
        ...
        NamedTypeAnnotationCode(
            name: objectFactoryStructureProxy, typeArguments: [
          NamedTypeAnnotationCode(name: clazz.identifier)
        ]),
        "(",
        "\$activator, [",
        ...structureFields
            .map((e) => "\$get_${e.identifier.name}")
            .commaDelimited,
        "], \$values));",
      ]));
    } catch (ex, st) {
  ...
  Future<PrimitiveRecord> resolveSimpleTypes(
      DefinitionPhaseIntrospector intro) async {
    var futures = [
  ...
  Future<bool> isSimpleType(TypeAnnotationCode code,
      DefinitionPhaseIntrospector intro, PrimitiveRecord record) async {
  ...
  Future<Code> createMetaList(Iterable<MetadataAnnotation> metadata,
      StaticType retainedAnnotationType, DefinitionBuilder builder) async {
    var parts = <Object>["["];
...
// currently work)
class ModelModule implements LibraryTypesMacro {
...
Code recreateAnnotation(MetadataAnnotation annotation) {
  ...
  throw ArgumentError.value(
      annotation, 'annotation', 'Unsupported annotation');
}

Future<List<FieldDeclaration>> getDogFields(ClassDeclaration clazz,
...
final _thisLibrary = Uri.parse('package:macro_test/dogs_gen.dart');
...
final _structureConverterFile = Uri.parse(
    'package:dogs_core/src/structure/converter.dart');
final _dogsGlobalFile = Uri.parse('package:dogs_core/src/global.dart');
final _dogsDatatypeOptionalFile = Uri.parse(
    'package:dogs_core/src/datatype/optional.dart');
